document.addEventListener('DOMContentLoaded', () => {
  ...
  // 5. Blackout project preview inside the portfolio page
  const blackoutPreviewTriggers = document.querySelectorAll('.blackout-preview-trigger');
  const blackoutPreview = document.getElementById('blackout-preview');
  const blackoutPreviewClose = document.getElementById('blackout-preview-close');
  const blackoutPreviewFrame = document.getElementById('blackout-preview-frame');

  if (blackoutPreviewTriggers.length && blackoutPreview && blackoutPreviewFrame) {
    let lastFocusedElement = null;

    const openBlackoutPreview = (trigger) => {
      lastFocusedElement = document.activeElement;
      const previewUrl = trigger?.getAttribute('href') || blackoutPreviewFrame.dataset.src;
      const previewTitle = trigger?.getAttribute('aria-label') || 'Предпросмотр проекта';

      blackoutPreview.classList.add('active');
      blackoutPreview.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
      blackoutPreviewFrame.title = previewTitle;
      blackoutPreviewFrame.src = previewUrl;

      setTimeout(() => {
        blackoutPreviewClose?.focus();
      }, 80);
    };

    const closeBlackoutPreview = () => {
      blackoutPreview.classList.remove('active');
      blackoutPreview.setAttribute('aria-hidden', 'true');
      document.body.style.overflow = '';
      blackoutPreviewFrame.src = '';
    ...
    blackoutPreviewTriggers.forEach(trigger => {
      trigger.addEventListener('click', (e) => {
        e.preventDefault();
        openBlackoutPreview(trigger);
      });
    });

    blackoutPreviewClose?.addEventListener('click', closeBlackoutPreview);

    blackoutPreview.addEventListener('click', (e) => {
      if (e.target === blackoutPreview) {
        closeBlackoutPreview();
      }
    });

    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && blackoutPreview.classList.contains('active')) {
        closeBlackoutPreview();
      }
    });
  }
